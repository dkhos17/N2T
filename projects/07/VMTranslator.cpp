...
int main(int argc, char** argv) {
    // ------- if need to compile as ./a.out path/xxx.vm ----------------------------
    
    // // check if arguments contains filename
    // assert(argc == 2); fname = string(argv[1]).substr(0,string(argv[1]).size()-2);

    // freopen((fname + "vm").c_str(), "r", stdin);
